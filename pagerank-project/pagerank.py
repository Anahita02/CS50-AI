...
def transition_model(corpus, page, damping_factor):
    ...
    #does the page has outgoing links?
    dict_len = len(corpus.keys())
    pages_len = len(corpus[page])

    #no outgonig pages, chose a random page
    if len(corpus[page]) < 1:
        for key in corpus.keys():
            prop_dist[key] = 1 / dict_len
        #if we have an outging pages, we need to calculate the disribution
        else:
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for _ in range(n):
        if sample:
            ...
            sample = random.choice(dist_list, dist_weights, k=1)[0]
        else:
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while True:
        for page in corpus:
            ...
            difference = max([abs(new_dic[x] - old_dict[x]) for x in old_dict])

            if difference < 0.001:
                break
            else:
                old_dict = new_dic.copy()
